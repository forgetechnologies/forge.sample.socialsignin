 ...
 * @version 0.1
 */
angular.module('starter.twitterLib', [])

    .factory('TwitterLib', function ($rootScope, $q, $window, $http, myAppConfig,$ionicHistory) {
        ...
        var twitterKey = "vDSNkKQpfJEQqe4H9PxR6xsXl"; // This key is used for storing Information related
        ...
        var Twitter = {
                ...
                mediaupload: function (photobase64) {
                    var deferred = $q.defer();
                    return deferred.promise
                        .then(Twitter.verify().then(function (result) {
                                console.log("in media verified success");
                                
                                tUrl = 'https://upload.twitter.com/1.1/media/upload.json';
                                tParams = {
                                    'media_data': photobase64,
                                };
                                apiPostCall =  Twitter.apiPostCall({
                                    url: tUrl,
                                    params: tParams
                                });
                                deferred.resolve(apiPostCall);
                                return apiPostCall;

                            }, function (_error) {
                                deferred.reject(JSON.parse(_error.text));
                                console.log("in media " + _error.text);
                            }
                        )
                    );
                },
               ...
               var accessToken = localStorage.getItem(twitterKey);
                // console.log($rootScope.accessT.accessTokenSecret);
                // var obj={
                //     authenticity_token:accessToken.twitterKey,
                //     reliability_event:"",
                //     scribe_log:""
                // }
                ...
                //     data:{ 
                //         "authenticity_token":$rootScope.accessT.accessTokenSecret
                //     },
                //     headers:
                ...
                // $http.post('https://twitter.com/logout', { 'authenticity_token' : $rootScope.accessT.twitterKey });
                window.localStorage.clear();
                window.sessionStorage.clear();
                    window.localStorage.clear(twitterKey);
...
.constant('myAppConfig', {
  oauthSettings: {
    consumerKey: '0GYkAfBTCwHYF7uKXAsnId9Xv',
    consumerSecret: 'kk68mR68lCa5FMxvDz2YpenSTNS0JxrUPU04HNwE3xZhfqNzYp',
